...
async fn install_single(
    ...
    let replacement_path = if existing_path.exists() {
        let mut suffix = 0u32;
        let backup = loop {
            let candidate = existing_path.with_extension(format!(
                "coldbrew-replace-{}-{}",
                std::process::id(),
                suffix
            ));
    ...
    if let Err(err) = db
        .upsert_store_entry(&conn, &bottle_plan.file.sha256, store_size)
    ...
    if ctx.platform.os == Os::MacOS {
        ...
        if summary.relocated_files > 0 {
            ...
            let _permit = match ctx.codesign_semaphore.clone().acquire_owned().await {
                Ok(permit) => permit,
                Err(err) => {
                    let err =
    ...
    if let Err(err) = ctx
        ...
        .and_then(|_| ctx.cellar.update_opt_link(name))
    {
        rollback_replacement(
            ctx,
            name,
    ...
    if let (Some(old_sha), Some(new_sha)) =
        (old_sha.as_deref(), Some(bottle_plan.file.sha256.as_str()))
    {
        if old_sha != new_sha {
...
fn rollback_replacement(
    ...
    if let Some(backup) = backup {
        if let Err(err) = fs::rename(backup, &current) {
            ctx.output
                .debug(&format!("Failed to restore {} {}: {}", name, version, err));
        }
    }
